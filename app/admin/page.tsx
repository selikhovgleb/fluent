  ...
  return (
    <main>
      <header className="topbar profile-topbar">
        ...
        <div className="admin-user"><small>Signed in as</small><strong>{user.displayName}</strong></div>
        <span className="avatar profile-avatar-small">{initials(user.displayName)}</span>
      </header>
      ...
      <div className="page-shell admin-page">
        <section className="admin-intro">
          ...
          <div className={data.status === "connected" ? "db-status connected" : "db-status error"}><i /><div><strong>{data.status === "connected" ? "Database connected" : "Database needs attention"}</strong><span>{data.status === "connected" ? "D1 · live environment" : data.error}</span></div></div>
        </section>
        ...
        <div className="admin-grid">
          ...
          <aside className="admin-side">
            ...
            <article className="admin-card admin-access"><p className="eyebrow">ACCESS</p><h2>Private owner dashboard</h2><p>This page uses the current private-site sign-in boundary. Before public Google sign-in is enabled, we’ll replace this with an explicit administrator allowlist.</p><Link href="/profile">Account settings <span>→</span></Link></article>
          </aside>
        </div>
      </div>
    </main>
