...
CREATE INDEX `idx_correction_events_user_created` ON `correction_events` (`user_id`,`created_at`);--> statement-breakpoint
CREATE TABLE `users` (
	`id` text PRIMARY KEY NOT NULL,
	...
	`store_sentences` integer DEFAULT false NOT NULL,
	`created_at` text DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
--> statement-breakpoint
CREATE INDEX `idx_vocabulary_reviews_user_reviewed` ON `vocabulary_reviews` (`user_id`,`reviewed_at`);--> statement-breakpoint
